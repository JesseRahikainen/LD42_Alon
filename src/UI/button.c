...
void btn_SetEnabled( int idx, bool enabled )
{
	if( ( idx < 0 ) || ( idx >= MAX_BUTTONS ) ) {
		return;
	}

	buttons[idx].ignore = !enabled;
}
	...
	unsigned int camFlags, char layer, ButtonResponse pressResponse, ButtonResponse releaseResponse )
{
	...
	newIdx = 0;
	while( ( newIdx < MAX_BUTTONS ) && ( buttons[newIdx].inUse ) ) {
		++newIdx;
	}
...
void btn_Draw( void )
{
	...
	for( i = 0; i < MAX_BUTTONS; ++i ) {
		if( buttons[i].inUse && !buttons[i].ignore ) {
...
void btn_Process( void )
{
	...
	for( int currCamera = cam_StartIteration( ); currCamera != -1; currCamera = cam_GetNextActiveCam( ) ) {
		...
		for( i = 0; i < MAX_BUTTONS; ++i ) {
			if( !buttons[i].inUse ) continue;
			if( ( buttons[i].camFlags & camFlags ) == 0 ) continue;
			if( buttons[i].ignore ) continue;
